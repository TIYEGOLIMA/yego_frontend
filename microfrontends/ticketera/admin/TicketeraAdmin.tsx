...
import { createSingleKeyListCache } from '../agentpanel/utils/listMemoryCache'
...
const usuariosSacCache = createSingleKeyListCache<UsuarioApi[]>()

const cargarUsuariosSacDeduped = (forzarRed: boolean = false): Promise<UsuarioApi[]> => {
  if (forzarRed) usuariosSacCache.invalidate()
  return usuariosSacCache.getOrFetch(() =>
    api
...
const DispositivosTab: React.FC<{ sedes: Sede[] }> = ({ sedes }) => {
  ...
  const cargar = async (forzarRed = false) => {
    setLoading(true)
    setError(null)
    try {
      if (forzarRed) dispositivosService.invalidateListCache()
      const data = await dispositivosService.listar()
      setItems(data)
    } catch (e: any) {
      setError(e?.response?.data?.message ?? 'Error cargando dispositivos')
    } finally {
      setLoading(false)
    }
  }
  ...
  const ejecutarRegenerar = async (d: Dispositivo) => {
    ...
    try {
      ...
      setError(e?.response?.data?.message ?? 'Error regenerando token')
    } finally {
      setAccionando(null)
    }
  }
  ...
  const ejecutarDesactivar = async (d: Dispositivo) => {
    ...
    try {
      ...
      setError(e?.response?.data?.message ?? 'Error desactivando dispositivo')
    } finally {
      setAccionando(null)
    }
  }
...
const DispositivoFormModal: React.FC<{
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(e?.response?.data?.message ?? 'Error guardando el dispositivo')
    } finally {
      setSaving(false)
    }
  }
...
const ModulosTab: React.FC<{ sedes: Sede[] }> = ({ sedes }) => {
  ...
  const cargar = async (forzarRed = false) => {
    ...
    try {
      if (forzarRed) {
        modulosAdminService.invalidateListCache()
        dispositivosService.invalidateListCache()
      }
      ...
      setError(e?.response?.data?.message ?? 'Error cargando módulos')
    } finally {
      setLoading(false)
    }
  }
  ...
  const ejecutarEliminar = async (m: ModuloAtencion) => {
    ...
    try {
      ...
      setError(e?.response?.data?.message ?? 'Error eliminando módulo')
    } finally {
      setAccionando(null)
    }
  }
...
const VincularTabletModal: React.FC<{
  ...
  const handleVincular = async () => {
    ...
    try {
      ...
      setError(e?.response?.data?.message ?? 'Error vinculando la tablet')
    } finally {
      setSaving(false)
    }
  }
...
const ModuloFormModal: React.FC<{
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(e?.response?.data?.message ?? 'Error guardando el módulo')
    } finally {
      setSaving(false)
    }
  }
...
const UsuariosSacTab: React.FC<{ sedes: Sede[] }> = ({ sedes }) => {
  ...
  const cargar = async (forzarRed = false) => {
    ...
    setError(null)
    try {
      const list = await cargarUsuariosSacDeduped(forzarRed)
      setItems(list)
    } catch (e: any) {
  ...
  const handleCambiarSede = async (userId: number, sedeId: number) => {
    ...
    try {
      ...
      setError(e?.response?.data?.message ?? 'Error actualizando la sede')
    } finally {
