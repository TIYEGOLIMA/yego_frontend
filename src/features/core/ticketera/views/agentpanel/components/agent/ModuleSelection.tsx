...
import { getSedeActiva, getSedeActivaId } from '@/features/core/ticketera/shared/utils/sedeContext'
...
interface ModulosUpdatePayload {
  sedeId?: number | null
  modulosOcupados?: ModuloPayload[]
  modulosDisponibles?: ModuloPayload[]
}
...
const updateUserModuleId = (moduleId: number) => {
  ...
  if (currentUser) useAuthStore.setState({ user: { ...currentUser, moduleId } })
}
...
const mapearModuloAtencion = (mod: ModuloPayload): ModuloAtencion => ({
  id: mod.id ?? mod.moduleId ?? 0,
  name: mod.name ?? mod.moduleName ?? '',
  description: mod.description ?? undefined
})

const mapearModuloOcupado = (mod: ModuloPayload): ModuloOcupado & { moduleName?: string } => ({
  moduleId: mod.moduleId ?? mod.id ?? 0,
  userId: mod.userId || 0,
...
const procesarModulosOcupados = (
  ...
  if (modulosDisponibles && Array.isArray(modulosDisponibles)) {
    modulosDisponibles.forEach((mod) => {
      modulosDisponiblesMap.set(mod.id ?? mod.moduleId ?? 0, mod.name ?? mod.moduleName ?? '')
    })
  }

  return modulosOcupados.map((mod) => {
    const ocupado = mapearModuloOcupado(mod)
    const moduleName = modulosDisponiblesMap.get(mod.moduleId ?? mod.id ?? 0)
    if (moduleName) {
      ocupado.moduleName = moduleName
    }
    return ocupado
  })
}
...
export const ModuleSelection: React.FC<ModuleSelectionProps> = ({ 
  ...
  useEffect(() => {
    const handleModulosActualizados = (data: ModulosUpdatePayload) => {
      if (data.modulosOcupados && Array.isArray(data.modulosOcupados)) {
    ...
    const sedeId = getSedeActivaId() ?? authUser?.sedeId
    if (sedeId == null) return
    return ticketeraRealtime.subscribeModules({ sedeId }, (event) => {
      if (event.type !== 'MODULES_UPDATED') return
      handleModulosActualizados({
        sedeId: event.sedeId,
        modulosOcupados: event.data.occupied,
        modulosDisponibles: event.data.available,
      })
    })
  }, [authUser?.sedeId])
  ...
  const handleModuleSelect = async (moduleId: number) => {
    ...
    try {
      ...
      setError(getTicketeraErrorMessage(error, 'Error al asignar el módulo'))
    } finally {
      setAssigning(null)
    }
  }
  ...
  const handleLiberarModulo = async (moduleId: number) => {
    try {
      ...
      setError(getTicketeraErrorMessage(error, 'Error al liberar el módulo'))
    } finally {
                  ...
                  {modulosOcupadosData.map((modulo) => (
                    ...
                    >
                      <CardHeader className="pb-3">
                        <div className="flex items-center justify-between mb-2">
                          <CardTitle className="text-lg font-bold text-gray-800 dark:text-white">
                            {(modulo as ModuloOcupado & { moduleName?: string }).moduleName || `Módulo ${modulo.moduleId}`}
                          </CardTitle>
