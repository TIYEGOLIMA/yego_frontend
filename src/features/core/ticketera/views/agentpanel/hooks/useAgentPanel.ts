...
interface UseAgentPanelReturn {
  ...
  actualizarModulosDesdeLista: (modules: ModuloAtencion[]) => void
}
...
export const useAgentPanel = (sedePickerKey?: number): UseAgentPanelReturn => {
  ...
  const [modules, setModules] = useState<ModuloAtencion[]>([])
  ...
  const perteneceASedeActiva = useCallback((message: { sedeId?: number | null }): boolean => {
  ...
  useEffect(() => {
    ...
    const ticketsSub = subscribe('/topic/tickets', (message: TicketSocketMessage) => {
    ...
    const ticketCalledSub = subscribe('/topic/ticket-called', (message: TicketSocketMessage) => {
    ...
    const ticketStartedSub = subscribe('/topic/ticket-started', (message: TicketSocketMessage) => {
      if (!message || message.type === 'MODULOS_ACTUALIZADOS' || !message.ticketId) {
        return;
      }
      if (!perteneceASedeActiva(message)) {
    ...
    const ticketCompletedSub = subscribe('/topic/ticket-completed', (message: TicketSocketMessage) => {
      if (!message || message.type === 'MODULOS_ACTUALIZADOS' || !message.ticketId) {
        return;
      }
      if (!perteneceASedeActiva(message)) {
    ...
    const newTicketSub = subscribe('/topic/new-ticket', (message: TicketSocketMessage) => {
  ...
  const cargarTickets = useCallback(async (esConsultaAutomatica = false) => {
    ...
    try {
      ...
      if (getTicketeraErrorStatus(error) === 401) {
  ...
  const atenderTicket = useCallback(async (ticket: Ticket) => {
    ...
    try {
      ...
      mostrarError(getTicketeraErrorMessage(error, 'Error al atender el ticket'))
    ...
    const inicializar = async () => {
      try {
        ...
        if (Array.isArray(respuesta)) {
          ...
          const respuestaObj = respuesta as ModuloUsuarioResponse
  ...
  }, [selectedModule, isConnected, cargarTickets]) 
  ...
  const liberarModulo = useCallback(async () => {
    try {
      ...
      if (currentUser) useAuthStore.setState({ user: { ...currentUser, moduleId: null } })
  ...
  const actualizarModulos = useCallback(async () => {
    try {
      ...
      let modulosParaActualizar: ModuloAtencion[] = []
      if (Array.isArray(respuesta)) {
        modulosParaActualizar = respuesta
      } else if (respuesta && typeof respuesta === 'object' && 'modulosDisponibles' in respuesta) {
        modulosParaActualizar = (respuesta as ModuloUsuarioResponse).modulosDisponibles || []
      }
  ...
  const actualizarModulosDesdeLista = useCallback((modules: ModuloAtencion[]) => {
    setModules(modules)
    hasLoadedModules.current = true
  }, [])
