...
const RatingTablet: React.FC = () => {
  ...
  const [dispositivo] = useState<DispositivoSession | null>(() => getDispositivoSession())
  const [isFullscreen, setIsFullscreen] = useState(false)
  const displayedTicketIdsRef = useRef(new Set<number>())
  ...
  const { isConnected, onTicketCompleted } = useRatingWebSocket()
  ...
  useEffect(() => {
    ...
    const unsubscribeCompleted = onTicketCompleted((ticket: unknown) => {
      const completedTicket = normalizeTicket(ticket)
      if (displayedTicketIdsRef.current.has(completedTicket.id)) return
      displayedTicketIdsRef.current.add(completedTicket.id)
      if (displayedTicketIdsRef.current.size > 100) {
        const oldestTicketId = displayedTicketIdsRef.current.values().next().value
        if (oldestTicketId != null) displayedTicketIdsRef.current.delete(oldestTicketId)
      }
      setSelectedTicket({ ...completedTicket, status: 'COMPLETED' })
      setRating(0)
      setComment('')
    }, moduleIdStr)
  ...
  }, [isConnected, onTicketCompleted, moduleIdStr])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSubmitError('No se pudo enviar la calificación. Intente nuevamente.')
    } finally {
      setSubmitting(false)
    }
  }
